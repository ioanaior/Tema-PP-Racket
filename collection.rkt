...
; Definim macro-ul collection-cdr pentru a imita comportamentul stream-cdr
(define collection-cdr stream-rest)

(define collection-car stream-first)
...
(define (collection-filter pred coll)
  (stream-filter pred coll))

;; MAP
(define (collection-map func collection)
  (stream-map func collection))
