...
(define (first-branch st)
  ;; returnam primul element din suffix tree
  (stream-first st)
  )
...
(define (get-branch-label branch)
  ;; returnam primul element al branch-ului (varful)
  (car branch)
  )
...
(define (get-branch-subtree branch)
  ;; returnam toate elementele branch-ului fara primul (cdr)
  (cdr branch)
  )
...
(define (get-ch-branch st ch)
  (if (st-empty? st) #f
      ;; altfel trebuie returnata ramura daca exista
      ;; e primul caracter din first branch la fel cu ch?
      ;; daca da, returnam branch-ul respectiv
      (if (char=? (car (get-branch-label (first-branch st))) ch) (first-branch st)
          ;; altfel cautam un branch in restul arborelui
